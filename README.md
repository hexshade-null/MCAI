...
让一个 AI 玩家（glm-5.3-flash）以真实协议进入 Minecraft 1.21.11 服务器：@它聊天就会回复，**言出法随**（自然语言指挥它走路/砍树/挖矿/攻击/执行指令）、**完整物理模拟**（重力/跳跃/游泳/爬梯/摔落伤害/被击退会飞）、**自主战斗**（被打反击、低血逃跑吃食回血）、死亡自动重生、低饥饿自动吃——**纯本地端，无需服务器插件**；另支持离线/微软登录、自定义皮肤、Simple Voice Chat 语音。

## 真人化 & 自主行为（V2）

- **真人化执行**：反应延迟 100-400ms、攻击节奏按武器冷却（剑 0.625s/斧 1s）+ 抖动、自然转向瞄准、卡住自动跳+侧移脱困。
- **自主任务**：`@bot 砍棵树`（扫描→走近→从下往上砍→捡掉落）、`@bot 挖矿`、`@bot 找树/找矿`、`@bot 捡东西`。
- **战斗**：被打自动反击（"干嘛打我！"）、血量 30-50% 边退边吃、<30% 逃跑回血（"先溜了！"）——`survival.combat_auto` 开关。
- **挖掘按原版公式**：硬度×工具速度（wiki 数值+协议 id 实测校准），空手挖石 7.5s 无掉落、木镐 1.15s 掉圆石。
- **物理引擎**：50ms 重力/阻力/AABB 碰撞积分，摔落伤害真实化（36 格实测死亡），水中减速上浮、爬梯。

详见 [ARCHITECTURE_V2.md](ARCHITECTURE_V2.md) 与 [TEST_REPORT.md](TEST_REPORT.md)。
